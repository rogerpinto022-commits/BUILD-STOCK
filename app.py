...
# -------------------------------------------------------------
  ...
  if st.session_state.gavetas.empty:
    ...
    if st.session_state.gaveta_selecionada_ativa is None:
      ...
      # Formulário de Cadastro com campos um embaixo do outro
      with st.form("form_novo_registro"):
        st.markdown("### ➕ Novo Lançamento de Material")
        ...
        btn_salvar_mat = st.form_submit_button("💾 Salvar Lançamento na Gaveta")

        if btn_salvar_mat:
          if id_mat:
            total_calculado = float(qtd_palete) * float(entrada)
      ...
      st.markdown("---")
      st.subheader(
          "📊 Tabela de Lançamentos com Filtro de Unidade e Cálculo Automático"
      )
